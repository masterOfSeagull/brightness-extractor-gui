...
`requirements.txt` contains supported dependency ranges; `requirements-lock.txt` records the versions verified for the Windows release build.
...
Untagged 16-bit files retain 16-bit decoding precision. ICC-tagged files are converted to sRGB through Pillow's color-management path; this currently processes them as 8-bit RGBA and records both source and processing depths in `palette.json`.
